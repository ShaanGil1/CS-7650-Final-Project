...
from transformers import DistilBertConfig, DistilBertModel, DistilBertTokenizer, DistilBertForQuestionAnswering
from transformers.optimization import get_linear_schedule_with_warmup
from torch.optim import AdamW

from eval import eval_qa
from iterator import read_squad_examples, convert_examples_to_features
from model import DomainQA, DomainQAMotivational
...
class BaseTrainer(object):
    def __init__(self, args):
        ...
        self.tokenizer = DistilBertTokenizer.from_pretrained("distilbert-base-uncased", do_lower_case=args.do_lower_case)
        if args.debug:
    ...
    def make_model_env(self, gpu, ngpus_per_node):
        ...
        num_warmup_steps = self.args.warmup_proportion * num_train_optimization_steps
        self.scheduler = get_linear_schedule_with_warmup(self.optimizer, num_warmup_steps, num_train_optimization_steps)
    ...
    def get_features(self, train_folder, debug=False):
        pickled_folder = self.args.pickled_folder + "_{}_{}".format("distilbert-base-uncased", str(self.args.skip_no_ans))
        ...
        for epoch in range(self.args.start_epoch, self.args.start_epoch + self.args.epochs):
            ...
            for data_loader, sampler in iter_lst:
                ...
                for i, batch in enumerate(data_loader, start=1):
                    ...
                    loss = self.model(input_ids, attention_mask=input_mask, start_positions=start_positions, end_positions=end_positions)[0]
                    loss = loss.mean()
    ...
    def evaluate_model(self, epoch):
        ...
        for dev_file in self.dev_files:
            ...
            fw.write("{} : {}  {}\n".format(file_name, f1, exact_match))
...
class AdvTrainer(BaseTrainer):
    ...
    def make_model_env(self, gpu, ngpus_per_node):
        ...
        self.qa_optimizer = AdamW(list(self.model.bert.parameters()) + list(self.model.qa_outputs.parameters()), lr=self.args.lr)
        self.dis_optimizer = AdamW(self.model.discriminator.parameters(), lr=self.args.lr)
        num_warmup_steps = self.args.warmup_proportion * num_train_optimization_steps
        self.qa_scheduler = get_linear_schedule_with_warmup(self.qa_optimizer, num_warmup_steps, num_train_optimization_steps)
        self.dis_scheduler = get_linear_schedule_with_warmup(self.dis_optimizer, num_warmup_steps,
        ...
        for epoch in range(self.args.start_epoch, self.args.start_epoch + self.args.epochs):
            ...
            for data_loader, sampler in iter_lst:
                ...
                for i, batch in enumerate(data_loader, start=1):
                    ...
                    qa_loss = self.model(input_ids, input_mask,
                    ...
                    dis_loss = self.model(input_ids, input_mask,
...
class MotivationalTrainer(BaseTrainer):
    def __init__(self, args):
        super(MotivationalTrainer, self).__init__(args)
    ...
    def make_model_env(self, gpu, ngpus_per_node):
        ...
        self.model = DomainQAMotivational(self.args.bert_model, self.args.num_classes,
                              self.args.hidden_size, self.args.num_layers,
                              self.args.dropout, self.args.dis_lambda,
                              self.args.concat, self.args.anneal)
        ...
        self.qa_optimizer = AdamW(list(self.model.bert.parameters()) + list(self.model.qa_outputs.parameters())
                                                                            + list(self.model.fc.parameters()), lr=self.args.lr)
        self.dis_optimizer = AdamW(self.model.discriminator.parameters(), lr=self.args.lr)
        self.motivational_optimizer = AdamW(self.model.motivational.parameters(), lr=self.args.lr)
        num_warmup_steps = self.args.warmup_proportion * num_train_optimization_steps
        self.qa_scheduler = get_linear_schedule_with_warmup(self.qa_optimizer, num_warmup_steps, num_train_optimization_steps)
        self.dis_scheduler = get_linear_schedule_with_warmup(self.dis_optimizer, num_warmup_steps,
                                                            num_train_optimization_steps)
        self.motivational_scheduler = get_linear_schedule_with_warmup(self.motivational_optimizer, num_warmup_steps,
                                                             num_train_optimization_steps)
        ...
        for epoch in range(self.args.start_epoch, self.args.start_epoch + self.args.epochs):
            ...
            for data_loader, sampler in iter_lst:
                ...
                for i, batch in enumerate(data_loader, start=1):
                    ...
                    # update motivational discriminator
                    motivational_loss = self.model(input_ids, input_mask,
                                          start_positions, end_positions, labels, dtype="motivational",
                                          global_step=step)
                    motivational_loss = motivational_loss.mean()
                    motivational_loss.backward()
                    avg_motivational_loss = self.cal_running_avg_loss(motivational_loss.item(), avg_motivational_loss)
                    self.motivational_optimizer.step()
                    self.motivational_optimizer.zero_grad()
                    self.motivational_scheduler.step()
                    step += 1
                    if epoch != 0 and i % 2000 == 0:
                        result_dict = self.evaluate_model(i)
                        for dev_file, f1 in result_dict.items():
                    ...
                    batch_step += 1
                    msg = "{}/{} {} - ETA : {} - QA loss: {:.4f}, DIS loss: {:.4f}, MOTIVATIONAL loss: {:.4f}" \
                        .format(batch_step, num_batches, progress_bar(batch_step, num_batches),
                                eta(start, batch_step, num_batches),
                                avg_qa_loss, avg_dis_loss, avg_motivational_loss)
                    print(msg, end="\r")

            print("[GPU Num: {}, Epoch: {}, Final QA loss: {:.4f}, Final DIS loss: {:.4f}, Final MOTIVATIONAL loss: {:.4f}]"
                  .format(self.args.gpu, epoch, avg_qa_loss, avg_dis_loss, avg_motivational_loss))
