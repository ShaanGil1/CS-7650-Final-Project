...
from trainer import BaseTrainer, AdvTrainer, MotivationalTrainer
...
def worker(gpu, ngpus_per_node, args):
    if args.adv:
        ...
        print("running motivational training...")
        model = MotivationalTrainer(args)
    else:
...
def main(args):
    ...
    pickled_folder = args.pickled_folder + "_{}_{}".format("distilbert-base-uncased", str(args.skip_no_ans))
    pickled_folder = pickled_folder + "/SQUAD"
    ...
    if args.adv:
        ...
        name = "base"
    save_dir = os.path.join("./save", "{}_{}".format(name, time.strftime("%m%d%H%M")))
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    args.save_dir = save_dir

    result_dir = os.path.join("./result", "{}_{}".format(name, time.strftime("%m%d%H%M")))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--motiv", action="store_true", help="Use motivational training")
